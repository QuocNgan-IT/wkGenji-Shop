<?php
    $ma_loai_hang = $_GET['MaLoaiHang'];

    //Xác định danh mục rỗng
    $sql = "select *
            from hanghoa
            where MaLoaiHang='$ma_loai_hang'";
    $sql_1 = $conn_dathang->query($sql)->fetch_array();

    if( $sql_1==NULL ) {
        $ten_danh_muc = " này chưa được bổ sung !";
    }
    else{
        //Nếu không rỗng thì lấy TenLoaiHang theo danh mục (MaLoaiHang) đã chọn
        $sql_2 = "select TenLoaiHang
                from loaihanghoa
                where MaLoaiHang='$ma_loai_hang'";
        $sql_3 = $conn_dathang->query($sql_2)->fetch_array();
        $ten_danh_muc = $sql_3['TenLoaiHang'];
    }
    ...
    //
    $sql = "select MSHH,TenHH,QuyCach,Gia,MaLoaiHang 
            from hanghoa 
            where MaLoaiHang='$ma_loai_hang'
            order by MSHH desc
    ...
    echo "<div class='chi_muc'>Sản phẩm thuộc danh mục <br>$ten_danh_muc</div>";
            ...
            echo "<td colspan='5' align='center'>";
                echo "<div class='phan_trang'>";
                    for( $i=1;$i<=$so_trang;$i++ ) {
                        $link = "?thamso=xuat_san_pham&MaLoaiHang=".$_GET['MaLoaiHang']."&trang=".$i;
                        echo "<a href='$link'>";
                            echo $i; echo " ";
                        echo "</a>";
                    }
